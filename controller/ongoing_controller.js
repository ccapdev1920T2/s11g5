    ...
    if (req.session.curr_user || req.session.guest_user){
      ...
      if((paramRoundID == 0 && validRoundID == 0) || (paramRoundID == 1 && req.session.roundID)){
        ...
        if(roundID){
          var current_round;
          if(req.session.curr_user)
            current_round = req.session.curr_user.status;
          else if(req.session.guest_user)
            current_round = req.session.guest_user.status;
          if(current_round == 'Active' || current_round == roundID){
            /* Find the round */
                  ...
                  if(result.motion && result.gov && result.opp && result.adjudicator.full_name != "No User"){
                    await db.findMany(Team, {teamname:{$in:[result.gov.teamname,result.opp.teamname]}}, async function(matchTeams){
                      if(matchTeams){
                        ...
                        await db.findMany(User, {username:{$in:[result.adjudicator.username, govTeam.first.username, govTeam.second.username, govTeam.third.username, oppTeam.first.username, oppTeam.second.username, oppTeam.third.username]}}, async function(participants){
                          if(participants){
                            var adj = {status:'Active'};
                            var govOne = {status:'Active'}, govTwo = {status:'Active'}, govThree = {status:'Active'};
                            var oppOne = {status:'Active'}, oppTwo = {status:'Active'}, oppThree = {status:'Active'};
                            for(i = 0; i < participants.length; i++){
                              if(participants[i].username == result.adjudicator.username)
                                adj = participants[i];
                              else if(participants[i].username == govTeam.first.username)
                                govOne = participants[i];
                              else if(participants[i].username == govTeam.second.username)
                                govTwo = participants[i];
                              else if(participants[i].username == govTeam.third.username)
                                govThree = participants[i];
                              else if(participants[i].username == oppTeam.first.username)
                                oppOne = participants[i];
                              else if(participants[i].username == oppTeam.second.username)
                                oppTwo = participants[i];
                              else if(participants[i].username == oppTeam.third.username)
                                oppThree = participants[i];
                            }
            ...
            req.session.pagename = 'Join a Round';
            req.session.header = 'Join a Round';
            req.session.message = 'Cannot start round! You currently have a different Ongoing Debate Round.';
            req.session.link = '/dashboard';
            req.session.back = 'Dashboard';
            res.redirect('/message');
            res.end();
